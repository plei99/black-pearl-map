...
const DEFAULT_INPUT = "src/data/black-pearl-official-restaurants.json";
...
function printHelp() {
  console.log(`Geocode the official Black Pearl restaurant list with Google Geocoding API.

Usage:
  node scripts/geocode-black-pearl.mjs [options]

Options:
  --input=path        Input JSON file
  --output=path       Output JSON file
  --api-key=value     Google API key
  --limit=number      Only process the first N restaurants
...
async function main() {
  ...
  const restaurants = options.limit ? inputData.slice(0, options.limit) : inputData;
  const results = [];
  const counters = {
    ok: 0,
    zero: 0,
  ...
  for (let index = 0; index < restaurants.length; index += 1) {
    ...
    process.stdout.write(`[${index + 1}/${restaurants.length}] ${query}\n`);
    ...
    if (options.delayMs > 0 && index < restaurants.length - 1) {
      await sleep(options.delayMs);
    }
  }

  const finalResults = applyOverrides(results, overrides);
  ...
  console.log(`Geocoded OK: ${counters.ok}`);
  console.log(`Zero results: ${counters.zero}`);
